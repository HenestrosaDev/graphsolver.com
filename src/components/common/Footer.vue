		...
		<div class="max-w-5xl mx-auto px-4 md:px-8 py-6">
			<div class="flex flex-col md:flex-row justify-between items-center gap-4">
				<div class="text-sm text-slate-500 dark:text-slate-300 text-center md:text-left">
					<span class="font-semibold text-slate-700 dark:text-slate-100">{{ t('common.appNamePro') }}</span>
					&copy; {{ currentYear }}
				...
				<div class="flex items-center gap-4 text-sm font-medium text-slate-500 dark:text-slate-300">
					<a href="#" class="hover:text-blue-600 dark:hover:text-blue-300 ">{{ t('footer.help') }}</a>
					<a
					...
					<span class="flex items-center gap-1.5 opacity-80">
						{{ t('footer.madeWith') }} <span class="text-red-500 animate-pulse">❤</span> {{ t('footer.and') }}
						<a
